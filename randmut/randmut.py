...
def randmut(fun,
    ...
    while (stall < n_stall) and (iteration < max_iter):
        ...
        for i in range(0, n_pop):
            ...
            status = Status(iteration=iteration, time=elapsed(), last_best_f=last_best_f,
...
def randmut_par(fun,
                ...
                eps=1e-6,
                maxiterations=1000,
                include_ancestor=True,  # Guarantee monotonicity of result for non-stochastic functions
                disp=True,
                disp_multiline=True,  # Display status in a multilines or permanent single line
                callback=None,
                b=10):
    """
    ...
    :param n_des: number of descendants
    :param p_max: maximal magnitude of mutation
    :param p_min: minimal magnitude of mutations
    :param maxmut: maximal number of mutations
    :param n_stall: number of "non-changing" (<eps) iterations to stop
    :param eps: precision of "non-changing" iteration
    :param maxiterations: maximal number of iterations
    :param include_ancestor: add ancestor to its descendants, guarantee monotonicity for non-stochastic fun
    :param disp: display information
    :param disp_multiline: multiline or single-line regime of displaing info
    :param b: base of power of mutation
    :return: ResultRM
    """
    ...
    def gen_descendant(x):
        d = x.copy()
        mutnumb = rnd.randint(1, maxmut + 1)
        for _ in range(0, mutnumb):
            pos = rnd.randint(0, n)
            d[pos] += scale * sb[pos] * b ** rnd.randint(p_min, p_max + 1) * 2 * (rnd.rand() - 0.5)
        d = bound(d)
    ...
    while (stall < n_stall) and (iteration < maxiterations):
        iteration += 1
        for i in range(0, n_pop):
            ancestor = population[i]
            descendants = list(gen_descendant(ancestor) for _ in range(n_des))
            ...
            # calculating values
            with Pool() as pool:
                fs = list(pool.imap(fun, descendants))
            # fs = [fun(d) for d in descendants]
            fevals += len(descendants)
